...
//	#define CAN_FILTER_MASK_END 	0xFFFFFFFF
	...
	uint8_t CC_CAN_StartCAN(FDCAN_HandleTypeDef*);
	uint8_t CC_CAN_EnableCANInt(FDCAN_HandleTypeDef*, uint32_t, uint32_t);
	void CC_CAN_SetRxAddress(const uint32_t);
	uint8_t CC_CAN_SendMessage(FDCAN_HandleTypeDef*, const FDCAN_TxHeaderTypeDef*, const uint8_t*);
